...
func TestGenerateSlug_DatabaseIntegration(t *testing.T) {
	...
	conv1, err := database.CreateConversation(ctx, nil, true, nil, nil, false)
	if err != nil {
	...
	conv2, err := database.CreateConversation(ctx, nil, true, nil, nil, false)
	if err != nil {
	...
	conv3, err := database.CreateConversation(ctx, nil, true, nil, nil, false)
	if err != nil {
