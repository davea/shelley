...
// choice persists to localStorage (shelley.thinkingLevel).
test.describe("Thinking level picker (PrimeVue)", () => {
  test("opens, lists levels, and selecting one persists the choice", async ({ page }) => {
    ...
    await page.evaluate(() => localStorage.removeItem("shelley.thinkingLevel"));
    await page.reload();
    await page.waitForLoadState("domcontentloaded");
    ...
    // All six reasoning levels are offered.
    ...
    expect(await page.evaluate(() => localStorage.getItem("shelley.thinkingLevel"))).toBe("high");
  });
});
